              ...
              <div className="flex min-w-min items-start gap-0">
                {stops.map((stop, index) => {
                  ...
                  const inner = (
                    <>
                      <span
                        className={`flex min-h-10 min-w-[3.5rem] items-center justify-center rounded-sm px-1.5 py-1 text-[9px] font-bold leading-tight tracking-wide uppercase sm:min-w-[4rem] sm:text-[10px] ${
                    ...
                    >
                      ...
                      {interactive && onSelectStop ? (
                        <button
                          type="button"
                          onClick={() =>
                            onSelectStop(selected ? null : stop.id)
                          }
                          className="relative z-10 flex w-[5rem] flex-col items-center text-center transition sm:w-28"
                          title={`${stop.name} · mi ${stop.mile}`}
                        >
                          {inner}
                        </button>
                      ) : (
                        <div className="relative z-10 flex w-[5rem] flex-col items-center text-center sm:w-28">
                          {inner}
                        </div>
                      )}
                    </div>
